-- ============================================================
-- Combined Database Fix Script
-- Run this in the Supabase SQL Editor (erosfuvpexeofhsjiefr)
-- ============================================================


-- ************************************************************
-- FIX 1: Recreate emergency_responses with correct schema
-- ************************************************************

-- Drop the old broken table (missing 8 columns the code expects)
DROP TABLE IF EXISTS public.emergency_responses CASCADE;

-- Recreate with full schema matching the codebase
...
CREATE INDEX idx_emergency_responses_emergency_id
  ON public.emergency_responses (emergency_id);
CREATE INDEX idx_emergency_responses_navigation_status
  ON public.emergency_responses (navigation_status);
CREATE INDEX idx_emergency_responses_choice
...
-- Seed: 10 dummy responses for dashboard testing
...
-- ************************************************************
-- FIX 2: Create missing ar_rooms table (for BlockShops page)
-- ************************************************************
...
CREATE POLICY "Allow all access on ar_rooms"
  ON public.ar_rooms FOR ALL USING (true) WITH CHECK (true);

-- Seed: sample rooms for testing the Block/Shops page
INSERT INTO public.ar_rooms (room_name, floor_no, is_active) VALUES
  ('Main Lobby',          'Ground Floor', 'Y'),
  ('Conference Room A',   'Ground Floor', 'Y'),
  ('Conference Room B',   'Ground Floor', 'Y'),
  ('Server Room',         'Ground Floor', 'N'),
  ('Cafeteria',           'Ground Floor', 'Y'),
  ('Security Office',     'Ground Floor', 'Y'),
  ('VIP Lounge',          '1st Floor',    'Y'),
  ('Media Room',          '1st Floor',    'Y'),
  ('Storage Room 1',      '1st Floor',    'N'),
  ('Executive Suite',     '1st Floor',    'Y'),
  ('Broadcast Studio',    '2nd Floor',    'Y'),
  ('Press Box',           '2nd Floor',    'Y'),
  ('Medical Bay',         '2nd Floor',    'Y'),
  ('Equipment Room',      '2nd Floor',    'N'),
  ('Observation Deck',    '2nd Floor',    'Y');


-- ============================================================
-- Done! Verify the fixes:
-- ============================================================
SELECT 'emergency_responses' AS fixed_table, count(*) AS rows FROM public.emergency_responses
UNION ALL
SELECT 'ar_rooms', count(*) FROM public.ar_rooms;
