...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const setActiveOrganization = useCallback((organization: { id: string; name: string; slug: string } | null) => {
    ...
    const fallbackToUserOrganization = stored.user.role !== "super_admin";
    const nextActiveOrganizationId = organization?.id ?? (fallbackToUserOrganization ? stored.user.organization_id : null);
    const nextActiveOrganizationName = organization?.name ?? (fallbackToUserOrganization ? stored.user.organization_name : null);
    const nextActiveOrganizationSlug = organization?.slug ?? (fallbackToUserOrganization ? stored.user.organization_slug : null);

    writeStoredSession({
      ...stored,
      activeOrganizationId: nextActiveOrganizationId,
      activeOrganizationName: nextActiveOrganizationName,
      activeOrganizationSlug: nextActiveOrganizationSlug,
    });

    setActiveOrganizationId(nextActiveOrganizationId);
    setActiveOrganizationName(nextActiveOrganizationName);
    setActiveOrganizationSlug(nextActiveOrganizationSlug);
  }, []);
