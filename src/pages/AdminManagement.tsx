...
import {
  ...
  deleteAdmin,
  deleteOrganization,
  getMattercraftConnectionDetails,
...
export default function AdminManagement() {
  ...
  const mattercraftConnection = getMattercraftConnectionDetails(activeOrganizationId);
  ...
  async function handleCopyMattercraftConfig() {
    if (!activeOrganizationId) return;

    const configSnippet = [
      "{",
      `  supabaseUrl: "${mattercraftConnection.supabaseUrl}",`,
      `  supabaseAnonKey: "${mattercraftConnection.supabasePublishableKey}",`,
      `  organizationId: "${activeOrganizationId}",`,
      `  label: "${activeOrganization?.name ?? activeOrganizationName ?? "Selected Client"}",`,
      "}",
    ].join("\n");

    await navigator.clipboard.writeText(configSnippet);
    toast.success("Mattercraft config copied.");
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Sign in as super admin, create client logins, and copy each client's organization ID for direct Supabase access"
        ...
        <div className="flex items-center justify-between gap-3 px-4 sm:px-5 py-3 border-b border-border bg-muted/30">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Each client gets its own organization ID, feature controls, and organization-scoped logins.
            </p>
          </div>
      ...
      <section className="grid grid-cols-1 xl:grid-cols-[1.4fr,1fr] gap-6">
        ...
        <div className="rounded-xl border border-border bg-card shadow-sm overflow-hidden">
          <div className="flex items-center justify-between gap-3 px-4 sm:px-5 py-3 border-b border-border bg-muted/30">
            <div>
              <h2 className="font-display font-semibold text-section-title text-foreground">Mattercraft Connection</h2>
              <p className="text-xs text-muted-foreground mt-0.5">
                Use the shared Supabase URL, publishable key, and this client's organization ID in Mattercraft. No Render API is required for this flow.
              </p>
            </div>
          </div>
          ...
          <div className="p-4 space-y-4">
            {!activeOrganizationId ? (
              <div className="text-sm text-muted-foreground">
                Select a client first to view its direct Mattercraft connection settings.
              </div>
            ) : (
              <div className="space-y-3">
                <div className="rounded-lg border border-border p-3">
                  ...
                  <div className="mt-3 flex justify-end">
                    <button
                      onClick={() => handleCopyAnyValue(activeOrganizationId, "Organization ID copied.")}
                      className="h-8 px-3 rounded-lg border border-border text-xs font-semibold text-foreground hover:bg-secondary/50 transition-colors flex items-center gap-2"
                    >
                ...
                <div className="rounded-lg border border-border p-3">
                  ...
                  <code className="mt-2 block break-all text-xs text-foreground">
                    {mattercraftConnection.supabaseUrl}
                  </code>
                </div>
                ...
                <div className="rounded-lg border border-border p-3">
                  ...
                  <code className="mt-2 block break-all text-xs text-foreground">
                    {mattercraftConnection.supabasePublishableKey}
                  </code>
                </div>

                <div className="rounded-lg border border-border bg-muted/20 p-4">
                  <p className="text-xs text-muted-foreground mb-2">
                    Example Mattercraft tenant config
                  </p>
  ...
  label: "${activeOrganization?.name ?? activeOrganizationName ?? "Selected Client"}"
                  ...
                  <div className="mt-3 flex justify-end">
                    <button
                      onClick={handleCopyMattercraftConfig}
                      className="h-8 px-3 rounded-lg border border-border text-xs font-semibold text-foreground hover:bg-secondary/50 transition-colors flex items-center gap-2"
                    >
                      <Copy className="w-3.5 h-3.5" />
                      Copy Config
                ...
                <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 p-3 text-xs text-muted-foreground">
                  Legacy organization API keys are no longer needed for the new direct Supabase architecture.
                </div>
              </div>
            )}
          </div>
        ...
        {organizationDeleteTarget && (
          <DialogShell title="Delete Client" onClose={() => setOrganizationDeleteTarget(null)}>
            <div className="space-y-4">
              ...
              <p className="text-sm text-muted-foreground">
                This removes the client logins, feature settings, and client-scoped dashboard data for this tenant. This cannot be undone.
              </p>
